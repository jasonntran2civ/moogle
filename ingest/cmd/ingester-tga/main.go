...
import (
	...
	"github.com/evidencelens/evidencelens/ingest/internal/ingesters/tga"
...
func main() {
	...
	defer cancel()
	logger := ingestcommon.MustNewLogger("ingester-tga")
	shutdown, _ := otel.Init(ctx, "ingester-tga")
	...
	defer pub.Close()
	ing := tga.New(logger, wm, arch, pub)
	runner := &ingestcommon.Runner{Source: "tga", Logger: logger, Run: ing.Run}
