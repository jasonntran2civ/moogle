...
import (
	...
	"github.com/evidencelens/evidencelens/ingest/internal/ingesters/pmcoa"
...
func main() {
	...
	defer cancel()
	logger := ingestcommon.MustNewLogger("ingester-pmc-oa")
	shutdown, _ := otel.Init(ctx, "ingester-pmc-oa")
	...
	defer pub.Close()
	ing := pmcoa.New(pmcoa.Config{MaxPerRun: ingestcommon.GetEnvInt("PMC_OA_MAX_PER_RUN", 5000)}, logger, wm, arch, pub)
	runner := &ingestcommon.Runner{Source: "pmc-oa", Logger: logger, Run: ing.Run}
