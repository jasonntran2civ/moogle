...
import (
	...
	"github.com/evidencelens/evidencelens/ingest/internal/ingesters/hpo"
...
func main() {
	...
	defer cancel()
	logger := ingestcommon.MustNewLogger("ingester-hpo")
	shutdown, _ := otel.Init(ctx, "ingester-hpo")
	...
	defer pub.Close()
	ing := hpo.New(logger, wm, arch, pub)
	runner := &ingestcommon.Runner{Source: "hpo", Logger: logger, Run: ing.Run}
