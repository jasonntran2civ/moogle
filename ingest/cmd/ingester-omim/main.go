...
import (
	...
	"github.com/evidencelens/evidencelens/ingest/internal/ingesters/omim"
...
func main() {
	...
	defer cancel()
	logger := ingestcommon.MustNewLogger("ingester-omim")
	shutdown, _ := otel.Init(ctx, "ingester-omim")
	...
	defer pub.Close()
	ing := omim.New(omim.Config{MaxPerRun: ingestcommon.GetEnvInt("OMIM_MAX_PER_RUN", 5000)}, logger, wm, arch, pub)
	runner := &ingestcommon.Runner{Source: "omim", Logger: logger, Run: ing.Run}
