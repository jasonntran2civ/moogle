// ingester-cochrane: Cochrane Library RSS + DOI metadata (spec section 5.1.11).
package main

import (
	"context"

	"github.com/evidencelens/evidencelens/ingest/internal/ingesters/cochrane"
...
func main() {
	...
	defer cancel()
	logger := ingestcommon.MustNewLogger("ingester-cochrane")
	shutdown, _ := otel.Init(ctx, "ingester-cochrane")
	...
	ing := cochrane.New(logger, wm, arch, pub)
	runner := &ingestcommon.Runner{Source: "cochrane", Logger: logger, Run: ing.Run}
