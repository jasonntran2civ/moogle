...
import (
	...
	"github.com/evidencelens/evidencelens/ingest/internal/ingesters/ema"
...
func main() {
	...
	defer cancel()
	logger := ingestcommon.MustNewLogger("ingester-ema")
	shutdown, _ := otel.Init(ctx, "ingester-ema")
	...
	defer pub.Close()
	ing := ema.New(logger, wm, arch, pub)
	runner := &ingestcommon.Runner{Source: "ema", Logger: logger, Run: ing.Run}
