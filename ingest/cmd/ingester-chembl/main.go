...
import (
	...
	"github.com/evidencelens/evidencelens/ingest/internal/ingesters/chembl"
...
func main() {
	...
	defer cancel()
	logger := ingestcommon.MustNewLogger("ingester-chembl")
	shutdown, _ := otel.Init(ctx, "ingester-chembl")
	...
	defer pub.Close()
	ing := chembl.New(chembl.Config{MaxPerRun: ingestcommon.GetEnvInt("CHEMBL_MAX_PER_RUN", 5000)}, logger, wm, arch, pub)
	runner := &ingestcommon.Runner{Source: "chembl", Logger: logger, Run: ing.Run}
