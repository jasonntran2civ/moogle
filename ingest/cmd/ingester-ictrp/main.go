// ingester-ictrp: WHO ICTRP weekly bulk XML (spec section 5.1.4).
package main

import (
	"context"

	"github.com/evidencelens/evidencelens/ingest/internal/ingesters/ictrp"
...
func main() {
	...
	defer cancel()
	logger := ingestcommon.MustNewLogger("ingester-ictrp")
	shutdown, _ := otel.Init(ctx, "ingester-ictrp")
	...
	ing := ictrp.New(logger, wm, arch, pub)
	runner := &ingestcommon.Runner{Source: "ictrp", Logger: logger, Run: ing.Run}
