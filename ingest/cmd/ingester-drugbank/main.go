...
import (
	...
	"github.com/evidencelens/evidencelens/ingest/internal/ingesters/drugbank"
...
func main() {
	...
	defer cancel()
	logger := ingestcommon.MustNewLogger("ingester-drugbank")
	shutdown, _ := otel.Init(ctx, "ingester-drugbank")
	...
	defer pub.Close()
	ing := drugbank.New(logger, wm, arch, pub)
	runner := &ingestcommon.Runner{Source: "drugbank", Logger: logger, Run: ing.Run}
