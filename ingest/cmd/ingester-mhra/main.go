...
import (
	...
	"github.com/evidencelens/evidencelens/ingest/internal/ingesters/mhra"
...
func main() {
	...
	defer cancel()
	logger := ingestcommon.MustNewLogger("ingester-mhra")
	shutdown, _ := otel.Init(ctx, "ingester-mhra")
	...
	defer pub.Close()
	ing := mhra.New(logger, wm, arch, pub)
	runner := &ingestcommon.Runner{Source: "mhra", Logger: logger, Run: ing.Run}
