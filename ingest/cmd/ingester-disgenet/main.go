...
import (
	...
	"github.com/evidencelens/evidencelens/ingest/internal/ingesters/disgenet"
...
func main() {
	...
	defer cancel()
	logger := ingestcommon.MustNewLogger("ingester-disgenet")
	shutdown, _ := otel.Init(ctx, "ingester-disgenet")
	...
	defer pub.Close()
	ing := disgenet.New(disgenet.Config{MaxPerRun: ingestcommon.GetEnvInt("DISGENET_MAX_PER_RUN", 5000)}, logger, wm, arch, pub)
	runner := &ingestcommon.Runner{Source: "disgenet", Logger: logger, Run: ing.Run}
