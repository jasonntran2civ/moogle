...
import (
	...
	"github.com/evidencelens/evidencelens/ingest/internal/ingesters/core"
...
func main() {
	...
	defer cancel()
	logger := ingestcommon.MustNewLogger("ingester-core")
	shutdown, _ := otel.Init(ctx, "ingester-core")
	...
	defer pub.Close()
	ing := core.New(core.Config{MaxPerRun: ingestcommon.GetEnvInt("CORE_MAX_PER_RUN", 5000)}, logger, wm, arch, pub)
	runner := &ingestcommon.Runner{Source: "core", Logger: logger, Run: ing.Run}
