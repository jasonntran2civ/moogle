...
import (
	...
	"github.com/evidencelens/evidencelens/ingest/internal/ingesters/healthcanada"
...
func main() {
	...
	defer cancel()
	logger := ingestcommon.MustNewLogger("ingester-health-canada")
	shutdown, _ := otel.Init(ctx, "ingester-health-canada")
	...
	defer pub.Close()
	ing := healthcanada.New(logger, wm, arch, pub)
	runner := &ingestcommon.Runner{Source: "health-canada", Logger: logger, Run: ing.Run}
