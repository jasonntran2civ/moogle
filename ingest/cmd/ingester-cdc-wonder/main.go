...
import (
	...
	"github.com/evidencelens/evidencelens/ingest/internal/ingesters/cdcwonder"
...
func main() {
	...
	defer cancel()
	logger := ingestcommon.MustNewLogger("ingester-cdc-wonder")
	shutdown, _ := otel.Init(ctx, "ingester-cdc-wonder")
	...
	defer pub.Close()
	ing := cdcwonder.New(logger, wm, arch, pub)
	runner := &ingestcommon.Runner{Source: "cdc-wonder", Logger: logger, Run: ing.Run}
